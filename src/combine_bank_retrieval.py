...
def combine_files_test():
    # The columns are [query_id, sent_id, sent_score, sent]
    data1 = [
        [1, 1, 1.23, "sent1"],
        [2, 4, 1.20, "sent4"],
        [2, 5, 1.54, "sent5"],
...
def combine_files(file_name_template: str, num_files: int, output_file: str, k: int):
    ...
    # Save combined result in disk. Only save sent_id and sentence with no header.
    df.to_csv(output_file, columns=[1, 3], header=False, index=False)
